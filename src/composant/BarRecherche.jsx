...
export default function BarRecherche({ isNavbar }) {
  const { recherche, setRecherche, montreRecherche, setMontreRecherche, produits } = useContext(ShopContext);
  ...
  const handleSuggestionClick = (produit) => {
    ...
    navigate(`/produit/${produit._id}`); // adapte la route selon ton app
  };

  // Si ce n'est pas dans la navbar et que montreRecherche est faux, ne rien afficher
  ...
  return (
    <div className={`flex items-center justify-center ${isNavbar ? 'w-full mx-10' : 'w-full py-5 bg-white border-b'}`}>
          ...
          <ul className="absolute top-full left-0 w-full bg-white border border-gray-200 rounded-b-md shadow-lg z-50 max-h-60 overflow-y-auto">
            {suggestions.map(p => (
              <li
                ...
                onMouseDown={(e) => e.preventDefault()} // empêche le blur de fermer avant le clic
                onClick={() => handleSuggestionClick(p)}
              >
                <img src={p.img[0]} alt={p.name} className="w-8 h-8 object-cover rounded-full border" />
      ...
      {/* Bouton X uniquement sur la version mobile (overlay) */}
      {!isNavbar && (
        <X
          onClick={() => {
